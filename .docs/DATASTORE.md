...
const job = await apiClient.createRecord("recruitment_tool", {
  ...
  benefits: ["Health Insurance", "Remote Work", "401k Matching", "Professional Development Budget"],
  application_stages: [
...
const createdCandidates = await Promise.all(
  candidates.map((candidate) => apiClient.createRecord("recruitment_tool", candidate))
);
```
