...
export class ApplicationService {
  ...
  async createApplication(appRequest: CreateApplicationRequest, createdBy: string): Promise<ApplicationRecord> {
    ...
    if (appRequest.default_role_id) {
      ...
      if (!role) {
        throw new RBACError(`Default role ${appRequest.default_role_id} not found`, RBACErrorCode.ROLE_NOT_FOUND);
      }
    }
   ...
   */
  async updateApplication(appId: string, updates: Partial<ApplicationRecord>, updatedBy: string): Promise<ApplicationRecord> {
    ...
    if (updates.default_role_id) {
      ...
      if (!role) {
        throw new RBACError(`Default role ${updates.default_role_id} not found`, RBACErrorCode.ROLE_NOT_FOUND);
      }
    }
  ...
  async deleteApplication(appId: string, deletedBy: string): Promise<void> {
    ...
    const usersWithAppRoles = allUsers.filter(user => 
      user.app_roles[appId] && user.app_roles[appId].length > 0
    );
  ...
  async getResolvedApplication(appId: string): Promise<ResolvedApplication | null> {
    ...
    if (app.default_role_id) {
      defaultRole = await apiClient.getRole(app.default_role_id);
    }
   ...
   */
  async getApplicationPermissions(appId: string): Promise<import('@/types/rbac').PermissionRecord[]> {
